...
public class AlphaBeta implements ComputerStrategy {
	...
	/**
	 * Constructor of the AlphaBeta class
	 * @param board
	 * @param color
	 */
	public AlphaBeta(Board board, String color) {
		this.board = board;
		this.color = color;
	...
	/**
	 * Use mini-max algorithm with alpha beta pruning to determine the next move
	 * @param availables
	 * @return
	 */
	public Position getMove(List<Position> positions) {
		...
		for (int i = 0; i < positions.size(); i++) { // check for every legal position
			...
			double currentScore = minScore(availables, 0, alpha, beta);  
			/* update best? */
	 ...
	 * @return
	 */
	private double minScore(List<Position> availables, int index, double alpha, double beta) {
		if (index == availables.size() - 1) return copy.evaluate(color);
			...
			for (int i = index; i < availables.size(); i++) {
				...
				pawn.setPawn(position.getX(), position.getY());
				copy.setMove(pawn);
				score = maxScore(availables, i + 1, alpha, beta);
				if (score < beta){
					beta = score;
				} 
				/* alpha cut-off */
	 ...
	 * @return
	 */
	private double maxScore(List<Position> availables, int index, double alpha, double beta) {
		if (index == availables.size() - 1) return copy.evaluate(color);
		else {
			double score = Double.MAX_VALUE;
			for (int i = index; i < availables.size(); i++) {
				...
				pawn.setPawn(position.getX(), position.getY());
				copy.setMove(pawn);
				score = minScore(availables, i + 1, alpha, beta);
				if (score > alpha){
					alpha = score;
				}
				/* beta cut-off */
